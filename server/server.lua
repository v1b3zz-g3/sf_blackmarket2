...
-- Multiple orders per player allowed — only total queue cap is enforced.
QBCore.Functions.CreateCallback("sf_blackmarket_sv:attemptOrder", function(src, cb, order, orderType)
    ...
    -- Count total active orders (server-wide cap)
    local numOrders = 0
    ...
    -- Contraband unlock check (async, nested to avoid race)
...
QBCore.Functions.CreateCallback("sf_blackmarket_sv:buyListing", function(src, cb, listingId)
    ...
    MySQL.single('SELECT * FROM sf_blackmarket_listings WHERE id = ? AND status = ?',
        ...
        function(row)
            ...
            -- Buyer's client spawns the container props
...
-- Helper: after props are (re)stored, restore correct phase targets
local function restoreGoodsPhaseForAll(gc, listingId)
    ...
    if not gc.sellerOpen and not gc.sealed then
        -- Phase 1: closed → "Open Container" on lock for all players
    ...
    elseif gc.sellerOpen and not gc.itemsLoaded and not gc.sealed then
        -- Phase 2: open, awaiting item deposit → update visual + "Deposit Items" for seller only
    ...
    elseif gc.sealed and not gc.isLooted then
        -- Phase 4: sealed → "Open Container" on crate for all players
        local crateEnt    = NetworkGetEntityFromNetworkId(gc.props.crate)
        local crateCoords = vec4(GetEntityCoords(crateEnt), GetEntityHeading(crateEnt))
...
-- Step 1: Buyer's client spawned the container → store props, restore phase
...
-- Step 2: Seller triggers "Open Container" on the lock
RegisterNetEvent("sf_blackmarket_sv:attemptOpenGoods", function(listingId)
    ...
    -- Play grinder animation on seller's client only
    TriggerClientEvent("sf_blackmarket_cl:openGoodsContainer", src, listingId, gc.props)
end)

-- Step 3: Seller's grinder animation finished → mark open, broadcast visual, add "Deposit Items" target
RegisterNetEvent("sf_blackmarket_sv:goodsContainerOpened", function(listingId)
    ...
    -- Remove open target from lock; update container collision/visual for all
    TriggerClientEvent("sf_blackmarket_cl:removeGoodsOpenTarget", -1, listingId)
    TriggerClientEvent("sf_blackmarket_cl:updateGoodsContainerOpen", -1, listingId, gc.props.container, gc.props.collision)

    -- Add "Deposit Items" target on crate for seller only
    local crateEnt    = NetworkGetEntityFromNetworkId(gc.props.crate)
    local crateCoords = vec4(GetEntityCoords(crateEnt), GetEntityHeading(crateEnt))
    TriggerClientEvent("sf_blackmarket_cl:addGoodsLoadTarget", src, listingId, crateCoords)
end)

-- Step 4a: Seller triggers "Deposit Items" → progress bar + open stash for them
...
-- Step 4b: Seller's deposit progress bar completed → remove items, save to stash, add "Seal" target
RegisterNetEvent("sf_blackmarket_sv:goodsLoadComplete", function(listingId)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or Player.PlayerData.citizenid ~= gc.sellerCid then return end
    if gc.itemsLoaded then return end  -- double-fire guard
    ...
    -- Save items into the buyer's stash immediately
...
-- Step 5a: Seller triggers "Seal Container"
...
RegisterNetEvent("sf_blackmarket_sv:goodsSealComplete", function(listingId)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or Player.PlayerData.citizenid ~= gc.sellerCid then return end
    if gc.sealed then return end  -- double-fire guard
...
-- Step 6: Buyer triggers "Open Container" on the sealed crate
...
-- Step 7: Buyer's loot animation finished → open stash
...
local function initPlayerOrders(src)
    ...
    for k, v in pairs(pendingOrders) do
        if v.cid == cid then
            ...
            -- Check if spawned props have gone stale (restart destroyed entities)
            if v.props then
                local propsDead = false
                for propKey, netId in pairs(v.props) do
                    if propKey ~= "lock" or not v.isOpen then  -- lock is deleted after open
                        local ent = NetworkGetEntityFromNetworkId(netId)
            ...
            if os.time() >= v.deliveryTime then
                if not v.props then
                    -- Container needs to be (re)spawned
                    if v.isOpen then
                        -- Was open before restart; reset so buyer can re-spawn
                        v.isOpen = false
    ...
    for lid, gc in pairs(goodsContainers) do
        ...
        -- Notify seller if their listing hasn't been sealed yet
        ...
        -- Notify buyer if container is sealed and waiting for pickup
        ...
        -- If props are nil (restart destroyed them), buyer must re-spawn
        if not gc.props then
            ...
            -- Phase targets will be restored once goodsPropsSpawned fires
        else
            -- Props are still alive; restore the correct phase target for this player
            if not gc.sellerOpen and not gc.sealed then
                -- Phase 1: "Open Container" on lock
                local lockEnt    = NetworkGetEntityFromNetworkId(gc.props.lock)
                local lockCoords = vec4(GetEntityCoords(lockEnt), GetEntityHeading(lockEnt))
                TriggerClientEvent("sf_blackmarket_cl:addGoodsOpenTarget", src, lid, lockCoords)

            elseif gc.sellerOpen and not gc.itemsLoaded and not gc.sealed then
                -- Phase 2: open, awaiting deposit
...
AddEventHandler('onResourceStart', function(resource)
        ...
        if rows then
            for _, row in ipairs(rows) do
                ...
                pendingOrders[idx] = {
                    ...
                    props          = nil,  -- always nil after restart; buyer re-spawns
    ...
    -- Recover goods orders (unsold → sealed-but-unlooted)
        ...
        if rows then
            for _, row in ipairs(rows) do
                if row.location_index and row.seal_deadline then
                    ...
                    -- Re-arm seal deadline timer (only if not yet sealed)
